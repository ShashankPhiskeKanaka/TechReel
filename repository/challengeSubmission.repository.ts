...
class ChallengeSubmissionRepository {
     ...
     */
    fetchAll = async (data: PaginationData, filters: {}, userId?: string, challengeId?: string): Promise<ChallengeSubmission[]> => {
    ...
    /**
     * Retrieves submissions specifically for a given challenge.
     * 
     * @description Filters submissions by challengeId with optional user-scoping. 
     * 
     * @param {string} challengeId - The unique identifier of the challenge.
     * @param {string} [userId] - Optional ID to filter submissions by a specific user.
     * @returns {Promise<ChallengeSubmission[]>} A collection of submissions for the challenge.
     */
    fetchByChallengeId = async (challengeId: string, userId?: string): Promise<ChallengeSubmission[]> => {
        const challengeSubmissions = await prisma.challenge_submissions.findMany({
            where: {
                challengeId,
                ...(userId ? {userId} : {}) 
     ...
     */
    delete = async (id: string, userId?: string): Promise<ChallengeSubmission> => {
